...
from app.models import User, Membership, Routine, FitnessProfile, Gym
from sqlalchemy import inspect, text, or_
from app.utils.tenant import slugify_gym_name
from app.utils.fitness import get_league_for_level
...
def ensure_fitness_social_tables():
    ...
    # Drop old FitnessRoutine/Exercise/RoutineExercise tables if they exist
    ...
    # Recreate WorkoutSession with new FK pointing to routine.id
    ...
    from app.models import League as _League, LeagueMember as _LM, FitnessProfile as _FP, WorkoutSession as _WS, BodyProgress as _BP
    # Recreate dropped tables
    db.create_all()
    # Auto-assign leagues for existing users
...
if __name__ == '__main__':
    ...
    with app.app_context():
        ...
        # Ensure the default gym is approved
        default_gym = Gym.query.order_by(Gym.id.asc()).first()
        if default_gym and not default_gym.approved:
            default_gym.approved = True
            db.session.commit()

        # Seed sample routines for admin's client
        admin_user = User.query.filter_by(username='admin').first()
        if admin_user:
        ...
        # Seed sample workout sessions for leaderboard / leagues
        from app.models import WorkoutSession as _WS, FitnessProfile as _FP, LeagueMember as _LM
        from app.utils.fitness import compute_level_from_xp
